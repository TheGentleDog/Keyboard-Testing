...
def calculate_pixel_error_report(
    ...
    *,
    ...
    for index in range(paired_count):
        ...
        valid_samples.append(PixelErrorSample(index, target, gaze_point, error_px))
...
def save_pixel_error_samples(report: dict, output_dir: str | Path = "pixel_error_results") -> Path:
    ...
    with samples_path.open("w", newline="", encoding="utf-8") as file:
        ...
        writer.writerow(["sample_index", "target_x", "target_y", "gaze_x", "gaze_y", "error_px"])
...
class GazePixelErrorRunner:
    ...
    def __init__(self, config: PixelErrorConfig, *, camera_id: int = 0):
        self.config = config
    ...
    def run(self) -> Optional[dict]:
        ...
        report = calculate_pixel_error_report(target_samples, gaze_points, config=self.config)
...
def main() -> None:
    ...
    if choice == "1":
        GazePixelErrorRunner(config).run()
        return
    ...
    report = calculate_pixel_error_report(targets, gaze_points, config=config)
    print_pixel_error_summary(report)
    report_path = save_pixel_error_report(report)
    samples_path = save_pixel_error_samples(report)
    graph_path = save_pixel_error_graph(report)
