"""
Filipino Dataset Generator using RoBERTa-Tagalog
Generates a fresh filipino_dataset.json from scratch using:
  jcblaise/roberta-tagalog-base (masked language model)

Fixes applied:
  - Use <mask> token (not [MASK])
  - Strip leading space from token_str before validation
  - Higher top_k=50 for richer vocabulary
  - More templates per category
  - Affix expansion for verb forms
  - Progress tracking
"""
...
TOP_K       = 50        # candidates per mask
MIN_SCORE   = 0.003     # lower = more words captured
OUTPUT_FILE = "filipino_dataset_roberta.json"

print(f"🤖 Loading {MODEL_NAME} ...")
fill = pipeline("fill-mask", model=MODEL_NAME, top_k=TOP_K)
print("✓ Model loaded.\n")

# ─────────────────────────────────────────────
# SEED TEMPLATES  (use <mask> — confirmed working)
# ─────────────────────────────────────────────
TEMPLATES = {
    "pronouns": [
        "<mask> ay gutom na.",
...
for category, templates in TEMPLATES.items():
    ...
    for tmpl in templates:
        ...
        try:
            results = fill(tmpl)
            for r in results:
                token = r["token_str"].strip().lower()  # FIX: strip leading space
                score = r["score"]
                if score < MIN_SCORE:
                    continue
                ...
                phrase = tmpl.replace("<mask>", token)
...
verb_base_words = list(category_words.get("verbs", Counter()).keys())[:40]
for base in verb_base_words:
...
print(f"\n✅ Saved → {OUTPUT_FILE}")
print("   Rename to 'filipino_dataset.json' to use with the keyboard.")
