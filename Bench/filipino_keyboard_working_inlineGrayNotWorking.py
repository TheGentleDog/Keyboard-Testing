...
class FilipinoKeyboard(tk.Tk):
    ...
    def backspace(self):
        """Remove last character from current input, or delete last word if input is empty"""
        if self.current_input:
            # Remove last character from current typing
            self.current_input = self.current_input[:-1]
            self.update_display()
            self.status_bar.config(text="Backspace")
        elif self.output_words:
            # If not typing, delete last word from output
            deleted_word = self.output_words.pop()
            self.update_display()
            self.status_bar.config(text=f"Deleted '{deleted_word}'")
        else:
