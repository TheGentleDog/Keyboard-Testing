...
class FilipinoKeyboard(tk.Tk):
    ...
    def create_widgets(self):
        ...
        self.text_display = tk.Text(text_frame, wrap="word", font=("Segoe UI", 14))
        self.text_display.pack(fill="both", expand=True)
        self.text_display.bind('<KeyRelease>', lambda e: self.update_suggestions())
        
        # Suggestions
        suggestions_frame = ttk.LabelFrame(self, text="Suggestions", padding="10")
        suggestions_frame.pack(fill="x", padx=10, pady=5)
        
        ttk.Label(suggestions_frame, text="Word Completion:", font=("Segoe UI", 9, "bold")).pack(anchor="w")
        self.completion_container = ttk.Frame(suggestions_frame)
        self.completion_container.pack(fill="x", pady=5)
        
        ttk.Label(suggestions_frame, text="Next Word:", font=("Segoe UI", 9, "bold")).pack(anchor="w", pady=(10,0))
        self.predictive_container = ttk.Frame(suggestions_frame)
        self.predictive_container.pack(fill="x", pady=5)
        
        # Virtual keyboard
        keyboard_frame = ttk.LabelFrame(self, text="Virtual Keyboard", padding="10")
        keyboard_frame.pack(fill="both", padx=10, pady=5)
    ...
    def create_keyboard(self, parent):
        ...
        style = ttk.Style()
        style.configure("Keyboard.TButton", font=("Segoe UI", 10), padding=4)
        ...
        for ch in "1234567890":
            ...
            btn.pack(side="left", ipadx=8, ipady=5, expand=True, fill="x", padx=1)
        ...
        for idx, row in enumerate(rows):
            ...
            for ch in row:
                btn = ttk.Button(row_frame, text=ch.upper(), style="Keyboard.TButton",
                               command=lambda c=ch: self.insert_char(c))
                btn.pack(side="left", ipadx=8, ipady=5, expand=True, fill="x", padx=1)
        
        # Bottom row
        bottom_row = ttk.Frame(parent)
        bottom_row.grid(row=5, column=0, sticky="ew", pady=2)
        
        backspace_btn = ttk.Button(bottom_row, text="⌫", style="Keyboard.TButton",
                                   command=self.backspace)
        backspace_btn.pack(side="left", ipadx=10, ipady=10, padx=2)
        
        space_btn = ttk.Button(bottom_row, text="SPACE", style="Keyboard.TButton",
                              command=self.space)
        space_btn.pack(ipadx=50, ipady=10, expand=True, fill="x")
        
        enter_btn = ttk.Button(bottom_row, text="↵", style="Keyboard.TButton",
                              command=self.enter)
        enter_btn.pack(side="right", ipadx=10, ipady=10, padx=2)
