...
class NgramModel:
    ...
    def resolve_shortcut(self, word):
        ...
        Checks: User shortcuts (learned from typing) > Dataset shortcuts
        """
        word_lower = word.lower()
        
        # Priority 1: User shortcuts (highest priority - user's personal patterns)
        if word_lower in self.user_shortcuts:
            return self.user_shortcuts[word_lower]
        
        # Priority 2: Dataset-learned shortcuts
    ...
    def get_completion_suggestions(self, prefix, context=None, max_results=8):
        ...
        if all_expansions:
            ...
            for full_word, source in all_expansions:
                # Higher priority for user shortcuts (learned from typing)
                priority_multiplier = 3.0 if source == 'user' else 1.5
