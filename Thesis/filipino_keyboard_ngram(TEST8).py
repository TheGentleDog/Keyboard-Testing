...
class NgramModel:
    ...
    def _learn_shortcuts_from_vocabulary(self):
        ...
        def create_initial_shortcut(word):
            
            if len(word) >= 4:
                # Simple heuristic: first + middle + last
                return word[0] + word[len(word)//2] + word[-1]
            return None
        
        # Analyze vocabulary to find potential shortcuts
        ...
        for word in self.vocabulary:
            ...
            if freq < 5:  # Only learn shortcuts for common words
                continue
            
            # Generate potential shortcuts
            shortcuts = []
            
            # Pattern 1: Remove vowels
            vowel_removed = remove_vowels(word)
            if vowel_removed and len(vowel_removed) < len(word):
            ...
            # Pattern 3: First few characters
            if len(word) >= 4:
                shortcuts.append(word[:2])  # First 2 chars
                shortcuts.append(word[:3])  # First 3 chars
            
            # Add shortcuts that are significantly shorter
            for shortcut in shortcuts:
                if len(shortcut) < len(word) - 1:  # At least 2 chars shorter
